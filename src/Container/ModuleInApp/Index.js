...
const itemSkus = Platform.select({
	...
	android: [
		//"com.inappdemonirmalsinh.audio_01" // purchase
		"audio_01" // purchase
	]
});
...
const itemSubs = Platform.select({
	...
	android: [
		"com.inappdemonirmalsinh.monthly" // subscription
	]
});
	...
	afterSetStateChangePurchase = async () => {
		try {
			...
			if (Platform.OS === "ios") {
				...
				this.onPressProduct(itemSkus[0]);
			}
		} catch (err) {
			this.setState({
				purchaseIndicator: false,
	...
	onPressProduct = async sku => {
		try {
			...
			if (Platform.OS === "android") {
				...
				alert(JSON.stringify(purchase.transactionReceipt))
			} else {
				this.receiptValidateIOS(purchase.transactionReceipt);
